...
class PayoneController extends Controller
{
    ...
    public function payone()
    {
        ...
        $istype =$_POST['istype'];
        $orderuid =$_SESSION['uid'].'&'.$pid.'&'.$_POST['num'];       //此处传入您网站用户的用户名，方便在paysapi后台查看是谁付的款，强烈建议加上。可忽略。
...
//返回正确
    ...
    public function paynotify()
    {
        ...
        if ($temps != $key) {
            ...
            $onemoney=M("onemoney");
            $result=$onemoney->where("id='$pid'")->setInc('number',$amount);//添加已被夺宝数量
            // $b['key']=$data['uid'].'&'.$data['pid'].'&'.$data['amount'].'&'.$orderuid.'&'.$res.'&'.$result.'&'.$data['paysapi_id'].'&'.$data['orderid'].'&'.$data['price'].'&'.$data['pay_money'].'&'.$data['orderuid'].'&'.$data['key'].'&'.$data['pay_time'];
            // $lootgoods->where("uid=38")->data($b)->save();
            if($res && $result){
                M()->commit();
            }else{
                M()->rollback();
...
//此处在您数据库中查询：此笔订单号是否已经异步通知给您付款成功了。如成功了，就给他返回一个支付成功的展示。
        $str="<a href=\"../Onemoney/onemoney\">点击返回一元夺宝</a>";
